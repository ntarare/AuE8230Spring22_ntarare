...
class LineFollower(object):
    ...
    def camera_callback(self, data):
        ...
        cv2.circle(mask,(int(cx), int(cy)), 6,(0,0,255),-1)
        ...
        if mode == 1:
            ...
            twist_object.angular.z = -err_x/500
            global stop
            global stop_once
            if stop == 1:
                if stop_once == 1:
...
def obstacle_avoidance_callback(laserscan): # defining a wall following function
    ...
    # scan = [laserscan[i] for i in range(len(laserscan)) if laserscan[i]<8] # taking only the values less than '8' 
    ...
    # front_dist = min(min(scan[0:15], scan[(len(scan)-15):len(scan)])) # front distance 
...
if __name__ == '__main__':
    ...
    while mode==0:
        ...
        rate.sleep()
    # april tag
    if stop == 0 and stop_once == 0:
                 
        #defining subscriber
        sub = rospy.Subscriber('/tag_detections',AprilTagDetectionArray,pose)
        x = pose.detections[0].pose.pose.pose.position.x
        z = pose.detections[0].pose.pose.pose.position.z
        move = Twist()
        k_linear_vel = 0.2
        k_angular_vel = 2
        #velocity controller 
        move.linear.x =z*k_linear_vel #desired linear velocity
        move.angular.z = -x*k_angular_vel #desired angular velocity
        #publishing velocity               
        rate.sleep()
